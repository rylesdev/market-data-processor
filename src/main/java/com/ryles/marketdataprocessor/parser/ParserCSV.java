...
public class ParserCSV implements Parser {
    ...
    public List<MarketData> parsing()
            throws AttributManquantException,
            AttributExcedantException,
            ChampManquantException,
            ValeurManquanteException {
        ...
        for (String foo : this.resultats) {
            ...
            if (chaines.length < 4) {
                throw new AttributManquantException(
                        "Il manque un attribut dans une ligne du fichier CSV"
                );
            }

            if (chaines.length > 4) {
                throw new AttributExcedantException(
                        "Il y a un ou plusieurs attributs en trop dans une ligne du fichier CSV"
                );
            }
            ...
            if (chaines[0].equals("date") ||
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (!(chaines[i].equals("date")) &&
                        ...
                        throw new ChampManquantException(
                                "Un champ du fichier CSV est inconnu"
                        );
                    }
                ...
                if (this.link.get("date") == null ||
                    ...
                    throw new ChampManquantException(
                            "Il manque un champ dans l'en-tête du fichier CSV"
                    );
                }
            ...
            if (this.link.get("date") == null ||
                ...
                throw new ChampManquantException(
                        "L'en-tête du fichier CSV est absent ou invalide"
                );
            }
            ...
            if (chaines[this.link.get("date")].isEmpty() ||
                ...
                throw new ValeurManquanteException(
                        "Il manque une valeur dans une ligne du fichier CSV"
                );
            }

            LocalDateTime date =
                    LocalDateTime.parse(chaines[this.link.get("date")]);

            String symbole =
                    chaines[this.link.get("symbole")];

            BigDecimal prix =
                    new BigDecimal(chaines[this.link.get("prix")]);

            long volume =
                    Long.parseLong(chaines[this.link.get("volume")]);
